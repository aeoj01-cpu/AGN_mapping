...
def temp0(dotmmlog, emlog, sigdotmmlog =0, sigemlog = 0.0, alpha_in = 0.75, sig_alphain=0.0,eta=0.1, alb=0, hxrs = 3,r0ld = 1):
 ...
 gnewt = 6.673e-11 # G Const
 c     = 2.9979e8 # Speed of light
 sig   = 5.67e-8 # Stefan-Boltzmann constant
 emsun   = 2.e30 # Solar mass
 ld    = 2.59e13 # Light day
 em    = 10**emlog # Mass in solar masses
 sigem = em*np.log(10)*sigemlog # Error in mass
 
 
 rs    = 2*gnewt*em*emsun/c**2 # Schwarzschild radius

 hx    = hxrs*rs # Height of the disk
 ...
 emsunyr = emsun/(3600.*24*365) # Solar mass per year
 
 dotmm = 10**dotmmlog # accretion rate in solar masses per year
 sigdotmm  = dotmm*np.log(10)*sigdotmmlog # error in accretion rate

 emdot = dotmm/em # accretion rate in Eddington units
 sigemdot = emdot*np.sqrt( (sigdotmm/dotmm)**2 + (sigem/em)**2 ) # error in accretion rate in Eddington units
  
  #T0^4 = 3GMMdot/(8*pi*sigma*r0^3) + hx(1 - a)Lbol/(4*pi*sigma*r0^3)
